...
sb = mcplates.SphericalBeta('sb', lon_lat=(mu_lon,mu_lat), alpha=alpha)
samples = np.array([sb.random() for i in range(1000)])
print samples
phi = samples[:,0]
theta = samples[:,1]
...
for i, lat in enumerate(reg_lat):
    for j,lon in enumerate(reg_lon):
        mesh_vals[i,j] = np.exp(mcplates.spherical_beta_logp( np.array([lon,lat]), np.array([mu_lon, mu_lat]), alpha ))
...
c = ax.pcolormesh(mesh_lon,mesh_lat, mesh_vals, transform=ccrs.PlateCarree())
